...
function checkArrayOrThrow(array) {
    if (!Array.isArray(array) || array.length === 0) {
        throw new Error('empty array');
    }
}

function checkFunctionOrThrow(fn) {
    if (!(fn instanceof Function)) {
        throw new Error('fn is not a function');
    }
}

function checker(array, fn, result) {
    checkArrayOrThrow(array)
    checkFunctionOrThrow(fn)
...
function isAllTrue(array, fn) {
    return checker(array, (item) => !fn(item), true);
}
...
function isSomeTrue(array, fn) {
    return checker(array, (item) => fn(item), false);
}
