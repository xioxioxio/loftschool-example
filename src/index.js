...
function findAllPSiblings(where) {
    return [].filter.call(where.childNodes, (child) => child.nextSibling && child.nextSibling.tagName === 'P');
}
...
function deleteTextNodes(where) {
    ...
    toRemove.forEach((value) => where.removeChild(value));
}
...
function collectDOMStat(root) {
    ...
    function keyPlus(obj, key) {
        typeof obj[key] === 'undefined' ? obj[key] = 1 : obj[key]++;
    }

    (function recursiveDOMStat(root, statObj) {
        for (var child of root.childNodes) {
            recursiveDOMStat(child, statObj);
            if (child.nodeType == Node.TEXT_NODE) {
                keyPlus(statObj, 'texts');
            } else {
                keyPlus(statObj.tags, child.tagName);
                child.classList.forEach((item) => keyPlus(statObj.classes, item));
            }
        }
    })(root, statObj);
...
function observeChildNodes(where, fn) {
    let mo = new MutationObserver((changes) => {
